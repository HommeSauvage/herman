...
import { createTab, openProject, openSession } from "../lib/agent-actions.js";
...
function SessionRow({
  ...
  return (
    ...
    >
      ...
      {isActive && <span className="text-signal text-[10px]">Active</span>}
    </button>
  );
}
...
export function HomeView() {
  ...
  const filteredSessions = useMemo(() => {
    const byProject = filterSessionsByProject(sessions, selectedProject);
    ...
    <div className="flex h-full w-full flex-col overflow-hidden">
      <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-4">
        ...
        <button
          ...
          disabled={!selectedProject && sessions.length === 0}
              ...
              <Section title="Today">
                {grouped.today.map((session) => (
                  <SessionRow
                    ...
                    isActive={session.id === activeTab?.id}
