...
export class AgentRpcClient {
  ...
  private feedStderrLog(chunk: string) {
    this.stderrLogLineBuffer += chunk;
    const lines = this.stderrLogLineBuffer.split("\n");
    this.stderrLogLineBuffer = lines.pop() ?? "";
    for (const line of lines) {
      this.logStderrLine(line);
    }
  }

  private flushStderrLog() {
    if (!this.stderrLogLineBuffer) return;
    this.logStderrLine(this.stderrLogLineBuffer);
    this.stderrLogLineBuffer = "";
  }
