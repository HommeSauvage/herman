...
/** Type representing a single file's diff information */
...
/** Strip leading/trailing quotes from a git-quoted path (e.g. `"a b"` → `a b`). */
...
/**
 * Parse a unified diff string into per-file structured diffs.
...
export function parseUnifiedDiff(diffText: string): ParsedFileDiff[] {
  ...
  for (const line of lines) {
    // Match git diff header: diff --git a/path b/path
    // Git quotes paths containing spaces, so handle both forms.
    const diffMatch = line.match(/^diff --git (?:a\/)?("?.+?"?) (?:b\/)?("?.+?"?)$/);
    if (diffMatch) {
      flush();
      currentFile = {
        path: unquotePath(diffMatch[2] ?? diffMatch[1] ?? ""),
        status: "modified",
        additions: 0,
        deletions: 0,
        patch: "",
    ...
    if (currentFile) {
      // Detect file status from mode/index lines
      ...
      // Count additions/deletions in hunk body
