...
> ## ⚡ Herman integration — read this first when running inside Herman Desktop
>
> When you are running as the Herman Desktop agent, the user has **already completed the Publishing setup** (server, SSH key, Coolify, API token) in the app's Publishing screen. Everything you need is one tool call away — **never ask the user for a Coolify URL, token, or server credentials in chat**.
>
> 1. **Get the connection details**: call `herman_get_publishing_config`. It returns the server IP, SSH key path, Coolify URL, and the Coolify API token (a secret — use it in commands, never repeat it in full). If it reports *no config*, stop and point the user to the **Publishing screen** instead of asking for credentials.
> 2. **Register the context once**: `coolify context add herman <coolifyUrl> <coolifyApiToken> -d`, then `coolify context verify`. Skip straight to operations when a context already exists.
> 3. **Create apps from git + Dockerfile only.** In Herman, always use the Dockerfile build pack (`coolify app create dockerfile …`, or a git source with the Dockerfile build pack). Do **not** use Nixpacks or docker-compose — the note about "prefer Nixpacks" in `references/deploy-patterns.md` is generic upstream guidance and does not apply here. If the project has no Dockerfile yet, create one for it first.
> 4. **Report back after every milestone** with `herman_update_publishing`: the real project UUID/name and application UUID right after creating them (`project_created`), the domain once assigned, and `deployed` once the app is live and reachable. The Publishing screen shows these to the user — keep them accurate, and always look UUIDs up with `coolify ... list --format=json`.
>
> Everything below is the generic upstream skill and still applies (safety rules, least-privilege tokens, decision trees).
