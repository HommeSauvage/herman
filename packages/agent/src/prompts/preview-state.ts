...
export function formatPreviewStateBlock(state: HostBridgePreviewState): string {
  ...
  if (state.phase === "ready") {
    ...
    const serverIdStr = state.primaryServerId
      ? ` (server "${state.primaryServerId}"${state.port ? `, port ${state.port}` : ""})`
      : "";
  ...
  if (server > 0 || consoleErrors > 0) {
    ...
    lines.push(
      `- Recent errors: ${parts.join(", ")} in the last 5 minutes — call herman_get_preview_logs("console" | "server") for details`,
    );
  }
